...
describe('WebsocketUrlHistoryModel - Events API', () => {
  ...
  describe(`${ArcModelEventTypes.WSUrlHistory.list} event`, () => {
    ...
    it('returns a query result for default parameters', async () => {
      const result = await ArcModelEvents.WSUrlHistory.list(et);
    ...
    it('respects "nextPageToken" parameter', async () => {
      const result1 = await ArcModelEvents.WSUrlHistory.list(et, {
        limit: 10,
      });
      const result2 = await ArcModelEvents.WSUrlHistory.list(et, {
  ...
  describe(`${ArcModelEventTypes.WSUrlHistory.insert} event`, () => {
    ...
    it('returns the changelog', async () => {
      const entity = generator.urls.url();
      const result = await ArcModelEvents.WSUrlHistory.insert(et, entity._id);
    ...
    it('creates an item in the data store', async () => {
      const entity = generator.urls.url();
      await ArcModelEvents.WSUrlHistory.insert(et, entity._id);
    ...
    it('throws when no url', async () => {
      let thrown = false;
      try {
        await ArcModelEvents.WSUrlHistory.insert(et, undefined);
      } catch (_) {
        thrown = true;
      }
    ...
    it('lowercases the _id', async () => {
      ...
      const result = await ArcModelEvents.WSUrlHistory.insert(et, url);
  ...
  describe(`${ArcModelEventTypes.WSUrlHistory.query} event`, () => {
    ...
    it('returns a list of matched results', async () => {
      const result = await ArcModelEvents.WSUrlHistory.query(et, 'http://');
    ...
    it('matches the URL', async () => {
      const result = await ArcModelEvents.WSUrlHistory.query(et, created[0]._id);
      assert.lengthOf(result, 1);
    });

    it('returns empty array when not found', async () => {
      const result = await ArcModelEvents.WSUrlHistory.query(et, 'this will not exist');
    ...
    it('adds midnight to an item when not there', async () => {
      ...
      delete entity.midnight;
      await instance.update(entity);
      const result = await ArcModelEvents.WSUrlHistory.query(et, entity._id);
      const [item] = result;
      assert.typeOf(item.midnight, 'number');
    });
    ...
    it('queries using lowercase keys', async () => {
      await ArcModelEvents.WSUrlHistory.insert(et, 'https://API.domain.com');
      const result = await ArcModelEvents.WSUrlHistory.query(et, 'https://api.DomaIN.com');
      const [item] = result;
      assert.equal(item.url, 'https://API.domain.com');
    });
  });
  ...
  describe(`${ArcModelEventTypes.destroy} event`, () => {
    ...
    it('deletes saved model', async () => {
      await ArcModelEvents.destroy(et, ['websocket-url-history'])
